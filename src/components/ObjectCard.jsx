...
export default function ObjectCard({ obj }) {
  ...
  const href = `/detail?id=${encodeURIComponent(obj.id)}`;

  return (
    <article className="group card-glass flex h-full flex-col">
      <Link
        href={href}
        className="relative m-2.5 mb-0 block overflow-hidden rounded-2xl bg-black/30 ring-1 ring-white/20"
      >
      ...
      <div className="flex flex-1 flex-col gap-2 px-4 pb-4 pt-3">
        <h3 className="text-base font-semibold tracking-tight text-white drop-shadow-sm sm:text-lg">
          <Link href={href} className="transition hover:text-white/90">
            {obj.name || "Tanpa nama"}
          </Link>
        </h3>
